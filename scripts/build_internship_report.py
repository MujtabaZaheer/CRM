...
def build_report():
    ...
    for section in doc.sections:
        ...
        # Footer for subsequent pages: Centered page number
    ...
    p_student.paragraph_format.space_after = Pt(32)
    ...
    r_to.font.size = Pt(13)

    p_sir = doc.add_paragraph()
    p_sir.alignment = WD_ALIGN_PARAGRAPH.CENTER
    p_sir.paragraph_format.space_after = Pt(32)
    r_sir = p_sir.add_run("MR.TAIMOOR RIAZ")
    r_sir.font.name = 'Times New Roman'
    r_sir.font.size = Pt(13.5)
    r_sir.font.bold = True

    # Duration
    p_dur = doc.add_paragraph()
    p_dur.alignment = WD_ALIGN_PARAGRAPH.CENTER
    p_dur.paragraph_format.space_after = Pt(38)
    r_dur = p_dur.add_run("Internship Duration-------------------6 Weeks")
    ...
    toc_entries = [
        ...
        ("Week 3: Counsellor Workspace, Lead Ingestion & Routing Engine", "11", False),
        ("Week 4: Application Pipeline, Admissions & Document Vault", "14", False),
        ("Week 5: Agent Partner Portal, University Desk & Finance Engine", "17", False),
        ("Week 6: AI Intelligence Suite, Testing, Optimization & Handover", "20", False),
        ("Conclusion", "23", True),
        ("References", "24", True),
    ]
    ...
    p_ack1.add_run(
        "I would like to express my sincere gratitude to the Director of Convo (Pvt.) Ltd., "
        "Mr. Sanaullah Irfan, for providing me with the opportunity to complete my full-stack software engineering "
        "internship at the organization's head office in Islamabad."
    )
    ...
    p_ack2.add_run(
        "I am deeply thankful for his visionary leadership, architectural guidance, and the supportive, high-performance "
        "professional environment fostered at Convo. Working under his mentorship has been an invaluable learning experience "
        "that significantly enhanced my technical competence across modern full-stack development, scalable multi-tenant architecture, "
        "and agentic AI integrations. His insights into product engineering, clean software design, and effective collaboration "
        "within an agile enterprise setting have been truly inspiring."
    )

    p_ack3 = doc.add_paragraph()
    p_ack3.paragraph_format.line_spacing = 1.25
    p_ack3.paragraph_format.space_after = Pt(14)
    p_ack3.add_run(
        "I also extend my heartfelt appreciation to my team lead, Mr. Waqar Akram, for his relentless technical mentorship, "
        "code reviews, and day-to-day guidance throughout the design and implementation of the Education CRM (EduCRM) platform. "
        "His practical recommendations on state management, asynchronous data flows, and rigorous testing methodologies were "
        "instrumental to my professional development."
    )

    p_ack4 = doc.add_paragraph()
    p_ack4.paragraph_format.line_spacing = 1.25
    p_ack4.paragraph_format.space_after = Pt(60)
    p_ack4.add_run(
        "This internship has played a crucial role in bridging my academic knowledge at Capital University of Science & Technology (CUST) "
        "with practical industry standards, giving me the confidence, problem-solving acumen, and technical depth required to "
        "excel as a software engineer."
    )
    ...
    p_sig2.paragraph_format.space_after = Pt(20)
    r_s2 = p_sig2.add_run("(Director Engineering & AI & Agentic Technology)")
    r_s2.font.size = Pt(11)

    p_sig3 = doc.add_paragraph()
    r_s3 = p_sig3.add_run("Signature : _________________________________")
    ...
    p_intro_body2.add_run(
        "During my 6-week summer internship at Convo, I was assigned to architect and develop core modules for the "
        "Education CRM and Student Application Management Platform (EduCRM). EduCRM is an enterprise-grade, multi-tenant "
        "web ecosystem tailored for international education consultants, global university recruitment networks, counsellors, "
        "and prospective students. The platform replaces fragmented, spreadsheet-driven operations with an integrated, intelligent, "
        "and automated operational backbone."
    )
    ...
    # ----------------------------------------------------
    # 5. DAILY ASSIGNMENT TASKS - WEEKS 1 TO 6
    # ----------------------------------------------------
    ...
    # Helper function for inserting image with border and caption
    def add_image_box(img_path, caption_text, width_inch=5.8):
    ...
    # WEEK 1
    # ----------------------------------------------------
    add_week_header("Week 1")

    add_day_entry(
        "13 July, 2026 — Monday",
        "On my first day at Convo (Pvt.) Ltd., I arrived at the head office in Islamabad for my full-stack software engineering "
        "internship. I attended a comprehensive orientation session conducted by HR and engineering management detailing the "
        "organization's culture, product portfolio, and engineering standards. During the session, I was introduced to the project "
        "I would be contributing to: the multi-tenant Education CRM and Student Application Management Platform (EduCRM). I set up my "
        "local development environment by configuring Node.js v22, Git version control, Visual Studio Code extensions, and cloning the project "
        "repository. I verified initial dependencies including React 19, Vite 6, Tailwind CSS v4, and Firebase client SDKs."
    )

    add_day_entry(
        "14 July, 2026 — Tuesday",
        "On my second day, I had the privilege of an architectural strategy session with the Director of Convo, Mr. Sanaullah Irfan. "
    ...
    add_day_entry(
        ...
        "I ensured that HTML5 client-side validation rules handled empty inputs and formatted error messages gracefully."
    )
    ...
    add_day_entry(
        ...
        "preventing unauthenticated users or unauthorized roles from accessing restricted platform areas. At the end of the day, I demonstrated "
        "the authentication workflows and role-switching capabilities to Mr. Waqar Akram, who approved the implementation."
    )
    ...
    add_day_entry(
        ...
        "wizard: personal profile, academic background, study preferences, and document submission checklist."
    )
    ...
    add_day_entry(
        ...
        "I implemented real-time validation schemas to prevent malformed score submissions."
    )
    ...
    add_day_entry(
        ...
        "recommendation, and Statements of Purpose (SOP)—and persisting onboarding completion flags in Firestore."
    )
    ...
    add_day_entry(
        ...
        "incorporating smooth transition animations and responsive cards."
    )
    ...
    add_day_entry(
        ...
        "and reviewed the completed student experience module with Mr. Waqar Akram."
    )
    ...
    add_day_entry(
        ...
        "Following the session, I began mapping out the counsellor workspace architecture."
    )
    ...
    add_day_entry(
        ...
        "allowing counsellors to manually ingest new walk-in leads with instant validation and branch assignment."
    )
    ...
    add_image_box(
        ...
        "Figure 3.3: Sequence Diagram — Automated Lead Lifecycle, Deduplication & Routing Flow"
    )
    ...
    add_day_entry(
        ...
        "with Mr. Waqar Akram, demonstrating lead ingestion, automated routing, and counsellor portfolio views. He commended the modularity "
        "of the routing rules engine."
    )
    ...
    add_day_entry(
        ...
        "Unconditional Offer, CAS / COE Issued, Visa Processing, and Final Enrolment."
    )
    ...
    add_image_box(
        ...
        "Figure 4.2: Architecture Diagram — Zero-Cost Cloud Storage & Document Ingestion Flow"
    )
    ...
    add_day_entry(
        ...
        "to the student portal with real-time notifications. I also validated immutable audit logging for all application state changes. I reviewed "
        "the completed admissions and document verification modules with Mr. Waqar Akram, who approved the integration."
    )

    add_image_box(
        os.path.join(os.getcwd(), 'diagrams_png', 'SD5_Application_Pipeline_and_Admissions.png'),
        "Figure 4.3: Sequence Diagram — University Application Processing & Admissions Desk Flow"
    )
    ...
    add_day_entry(
        ...
        "Portal and the University Partner Portal."
    )
    ...
    # ----------------------------------------------------
    doc.add_page_break()
    add_week_header("Week 6")

    add_day_entry(
        "17 August, 2026 — Monday",
        "At the start of the final week, Director Mr. Sanaullah Irfan led an advanced session on integrating agentic AI capabilities and large "
    ...
    add_day_entry(
        ...
        "high-conversion leads for counsellors."
    )
    ...
    add_day_entry(
        ...
        "than 48 hours, missing mandatory compliance files, and orphan applications. I also verified the immutable Audit Log Viewer (/audit-log), "
        "which records administrative actions, role impersonation events, and critical record edits to ensure GDPR compliance and institutional auditability."
    )

    add_image_box(
        os.path.join(os.getcwd(), 'crm_screenshots', '12_data_quality_dashboard.png'),
        "Figure 6.3: Data Quality, Audit Logs & Master Data Configuration Dashboard"
    )

    add_day_entry(
        "20 August, 2026 — Thursday",
        "Today, I dedicated my time to comprehensive end-to-end automated testing, debugging, and performance optimization. Working with "
        "Mr. Waqar Akram, I executed the Playwright automated test suites (tests/e2e/auth.spec.ts, student-workflow.spec.ts, ui-interactions.spec.ts), "
        "validating role permissions, UI responsiveness, theme toggling, and data synchronization under simulated high-concurrency loads. "
        "I resolved minor layout shifts on mobile viewports, optimized asset loading, and verified that all TypeScript types passed with zero errors."
    )

    add_image_box(
        os.path.join(os.getcwd(), 'diagrams_png', 'SSD_System_Sequence_Diagram.png'),
        "Figure 6.4: Comprehensive System Sequence Diagram (SSD) — End-to-End EduCRM Architecture"
    )

    add_day_entry(
        "21 August, 2026 — Friday",
        "On the final day of my 6-week internship, I delivered a comprehensive live demonstration of the completed Education CRM platform "
        "to Director Mr. Sanaullah Irfan and Team Lead Mr. Waqar Akram. I demonstrated the full end-to-end journey: from multi-role authentication, "
        "student onboarding, and university discovery to lead routing, application submission, document verification, external agent commissions, "
        "and Gemini AI eligibility parsing. Both mentors commended the technical depth, UI polish, and architectural integrity of the system. "
        "I organized the code repository, archived project documentation, completed formal exit formalities, and concluded my internship at Convo (Pvt.) Ltd."
    )
    ...
    p_concl_1.add_run(
        "My six-week full-stack software engineering internship at Convo (Pvt.) Ltd. has been an immensely transformative, rigorous, "
        "and professionally rewarding experience. Transitioning from academic coursework at Capital University of Science & Technology (CUST) "
        "into an enterprise product engineering environment allowed me to apply theoretical software engineering principles to a large-scale, "
        "production-grade software platform."
    )
    ...
    p_concl_2.add_run(
        "Under the visionary leadership of Director Mr. Sanaullah Irfan and the continuous technical mentorship of Mr. Waqar Akram, "
        "I gained profound practical expertise across modern web engineering paradigms. Architecting and implementing core components of the "
        "Education CRM and Student Application Management Platform (EduCRM) expanded my technical capabilities in React 19, TypeScript, "
        "Tailwind CSS v4, and Firebase Firestore, while teaching me the critical importance of multi-tenant security, data isolation, "
    ...
    p_concl_3.add_run(
        "A major highlight of my internship was integrating cutting-edge artificial intelligence into traditional business workflows. "
        "Working on Google Gemini AI integrations for automated CV/resume parsing, intelligent programme eligibility matching, and algorithmic "
        "lead scoring demonstrated firsthand how AI-assisted tooling can dramatically elevate business efficiency and user experience. Furthermore, "
        "engineering multi-tenant portals for external recruitment agents and partner universities provided me with valuable perspective on "
        "B2B software design and stakeholder-driven development."
    )
    ...
    p_concl_4.add_run(
        "Beyond technical coding skills, this internship refined my professional competencies in agile methodologies, collaborative Git version "
        "control workflows, automated testing with Playwright, and clear cross-functional communication. The mentorship, encouragement, and high "
        "standards maintained at Convo have established a strong foundation for my future career as a professional software engineer."
    )
    ...
    references = [
        ...
        ("2. Convo (Pvt.) Ltd. ", "Internal Training Documentation, Software Architecture Guidelines, Multi-Tenant Cloud Frameworks, and AI Integration Standards. Head Office, Islamabad, Pakistan."),
        ("3. EduCRM System Architecture & Flow Specifications. ", "Technical Documentation, UML Sequence Diagrams, and Requirements Traceability Matrix for the Education CRM and Student Application Management Platform. Convo (Pvt.) Ltd."),
        ("4. React Documentation. ", "Modern React 19 Architecture, Functional Components, Custom Hooks, and Reactive State Management. Official React Documentation (react.dev)."),
        ("5. TypeScript Documentation. ", "Static Type Systems, Interface Contracts, Generic Types, and Type-Safe Application Architecture. Microsoft TypeScript Documentation (typescriptlang.org)."),
        ("6. Tailwind CSS Framework. ", "Utility-First CSS Styling, Modern Responsive Layouts, and Dynamic Theme Switching. Official Tailwind CSS v4 Documentation (tailwindcss.com)."),
        ...
        ("8. Playwright Framework. ", "End-to-End Browser Automation, Cross-Browser Testing, and Resilient UI Integration Verification. Microsoft Playwright Documentation (playwright.dev)."),
        ("9. Google Gemini AI API. ", "Multimodal Large Language Models, Structured Document OCR Parsing, and Contextual Intelligence Integration. Google AI for Developers Documentation (ai.google.dev)."),
    ]
