...
const StatusBadge: React.FC<{ value: string }> = ({ value }) => <span className="px-2 py-0.5 sq-badge border border-emerald-500/20 bg-emerald-500/10 text-emerald-400 font-semibold">{value}</span>;
const Empty: React.FC<{ text: string }> = ({ text }) => <div className="p-10 text-center text-[var(--text-muted)] bg-[var(--bg-card)] border border-[var(--border-default)] sq-card">{text}</div>;
...
export const FinanceWorkspace: React.FC<{ page: FinancePage }> = ({ page }) => {
  ...
  const revenueTrend = useMemo(() => {
    ...
    finance.payments.forEach((payment) => { const month = payment.paidAt ? payment.paidAt.slice(0, 7) : "Unknown"; buckets.set(month, (buckets.get(month) || 0) + payment.amount); });
  ...
  const handlePrintReceipt = (pmt: Payment) => {
    const html = generateReceiptHtml(pmt);
    printDocumentHtml(html);
  };
  ...
  return (
    <div className="space-y-6 text-xs">
      ...
      <div
        ...
        style={{ backgroundImage: `url('/images/role_finance.jpg')` }}
      />
      ...
      <div className="relative z-10">
        <h1 className="text-2xl font-bold font-heading text-[var(--text-primary)] capitalize">Finance {page === "dashboard" ? "Dashboard" : page}</h1>
        <p className="text-[var(--text-secondary)] mt-1">Authorized financial operations, receipts, receivables, and compliance records.</p>
      </div>
      {["invoices", "payments", "refunds", "commissions"].includes(page) && (
        <button onClick={() => setShowForm(true)} className="relative z-10 flex items-center justify-center gap-2 px-4 py-2 bg-emerald-500 text-zinc-950 font-bold sq-btn cursor-pointer">
          <Plus className="w-4 h-4" />Record {page.slice(0, -1)}
        </button>
      )}
    </div>
...
const SearchBar: React.FC<{ value: string; setValue: (value: string) => void }> = ({ value, setValue }) => <div className="relative max-w-md"><Search className="absolute left-3 top-2.5 w-4 h-4 text-[var(--text-muted)]" /><input value={value} onChange={(event) => setValue(event.target.value)} placeholder="Search invoices..." className="w-full pl-9 p-2 bg-[var(--bg-input)] border border-[var(--border-default)] sq-input" /></div>;
const DataTable: React.FC<{ title: string; headers: string[]; rows: React.ReactNode[][] }> = ({ title, headers, rows }) => <div className="bg-[var(--bg-card)] border border-[var(--border-default)] sq-card overflow-hidden"><div className="p-4 font-bold text-sm text-[var(--text-primary)]">{title}</div>{rows.length === 0 ? <Empty text="No records available yet." /> : <div className="overflow-x-auto"><table className="w-full text-left"><thead className="bg-[var(--bg-elevated)] text-[var(--text-muted)] uppercase"><tr>{headers.map((header) => <th key={header} className="p-3">{header}</th>)}</tr></thead><tbody className="divide-y divide-[var(--border-default)]">{rows.slice(0, 10).map((row, index) => <tr key={index}>{row.map((cell, column) => <td key={column} className="p-3 text-[var(--text-secondary)]">{cell}</td>)}</tr>)}</tbody></table></div>}</div>;
const FinanceForm: React.FC<{ page: FinancePage; invoices: { id: string; invoiceNumber: string; studentName: string }[]; onSubmit: (event: FormEvent<HTMLFormElement>) => void; onClose: () => void }> = ({ page, invoices, onSubmit, onClose }) => <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-[var(--backdrop)]"><form onSubmit={onSubmit} className="w-full max-w-lg p-6 space-y-3 bg-[var(--bg-card)] border border-[var(--border-default)] sq-modal"><h2 className="font-bold text-base capitalize">Record {page.slice(0, -1)}</h2>{page === "invoices" ? <><Input name="invoiceNumber" label="Invoice number" required /><Input name="studentName" label="Student name" required /><Select name="type" label="Service" values={["Application Fee", "Service Charge", "Deposit", "Tuition Fee", "Visa Fee", "Other"]} /><Input name="dueDate" label="Due date" type="date" required /></> : page === "commissions" ? <><Input name="agentName" label="Agent name" required /><Input name="universityName" label="University" /></> : <><Select name="invoiceId" label="Invoice" values={invoices.map((invoice) => invoice.id)} labels={invoices.map((invoice) => `${invoice.invoiceNumber} - ${invoice.studentName}`)} /><Input name="studentName" label="Student name" required />{page === "payments" && <><Input name="invoiceNumber" label="Invoice number" required /><Select name="method" label="Payment method" values={["Card", "Bank Transfer", "Cash", "Online Gateway", "Cheque", "Other"]} /><Input name="reference" label="Reference number" required /><Input name="paidAt" label="Payment date" type="date" required /></>}{page === "refunds" && <Input name="reason" label="Refund reason" required />}</>}<div className="grid grid-cols-2 gap-3"><Input name="amount" label="Amount" type="number" required /><Input name="currency" label="Currency" defaultValue="USD" required /></div>{page === "invoices" && <Input name="notes" label="Notes" />}<div className="flex justify-end gap-2 pt-3"><button type="button" onClick={onClose} className="px-3 py-2 sq-btn bg-[var(--bg-hover)]">Cancel</button><button className="px-3 py-2 sq-btn bg-emerald-500 text-zinc-950 font-bold">Save</button></div></form></div>;
const Input: React.FC<{ name: string; label: string; type?: string; required?: boolean; defaultValue?: string }> = ({ name, label, type = "text", required, defaultValue }) => <label className="block text-[var(--text-secondary)]">{label}<input name={name} type={type} required={required} defaultValue={defaultValue} className="w-full mt-1 p-2 bg-[var(--bg-input)] border border-[var(--border-default)] sq-input" /></label>;
const Select: React.FC<{ name: string; label: string; values: string[]; labels?: string[] }> = ({ name, label, values, labels }) => <label className="block text-[var(--text-secondary)]">{label}<select name={name} className="w-full mt-1 p-2 bg-[var(--bg-input)] border border-[var(--border-default)] sq-input">{values.map((value, index) => <option key={value} value={value}>{labels?.[index] || value}</option>)}</select></label>;
