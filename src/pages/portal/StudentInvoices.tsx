  ...
  return (
    <div className="mx-auto max-w-7xl space-y-7 pb-16 font-sans animate-fade-in">
      ...
      <header className="rounded-3xl bg-surface/90 backdrop-blur-xl border border-subtle px-6 py-7 shadow-sm sm:px-8 relative overflow-hidden">
        ...
        <div className="relative z-10 flex flex-col md:flex-row md:items-end justify-between gap-4">
          <div>
            ...
            <p className="mt-1 text-xs sm:text-sm text-secondary max-w-2xl">
          ...
          <div className="flex items-center gap-2.5">
            <button
              ...
              className="px-3.5 py-2 rounded-xl bg-elevated hover:bg-elevated/80 border border-subtle text-xs font-semibold text-primary transition-all flex items-center gap-1.5 shadow-sm cursor-pointer"
            >
      ...
      <section className="grid grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-5">
        <div className="rounded-2xl bg-surface border border-subtle p-5 shadow-xs relative overflow-hidden">
          ...
          <p className="mt-2 text-2xl sm:text-3xl font-bold font-heading text-primary">
            {stats.totalCount}
          </p>
          <p className="mt-1 text-[11px] text-secondary">
            {stats.primaryCurrency} {stats.totalAmount.toLocaleString()} total billed
          </p>
        </div>
        ...
        <div className="rounded-2xl bg-surface border border-subtle p-5 shadow-xs relative overflow-hidden">
          ...
          <p className="mt-2 text-2xl sm:text-3xl font-bold font-heading text-amber-600 dark:text-amber-400">
            {stats.pendingCount}
          </p>
          <p className="mt-1 text-[11px] text-secondary">
            {stats.primaryCurrency} {stats.pendingAmount.toLocaleString()} deposit required
          </p>
        </div>

        <div className="rounded-2xl bg-surface border border-subtle p-5 shadow-xs relative overflow-hidden">
          <div className="flex items-center justify-between text-sky-600 dark:text-sky-400 text-xs font-bold uppercase tracking-wider">
            <span>Under Verification</span>
            <Upload className="w-4 h-4 text-sky-500" />
          </div>
          <p className="mt-2 text-2xl sm:text-3xl font-bold font-heading text-sky-600 dark:text-sky-400">
            {stats.verifyingCount}
          </p>
          <p className="mt-1 text-[11px] text-secondary">
            Receipts submitted & under review
          </p>
        </div>
        ...
        <div className="rounded-2xl bg-surface border border-subtle p-5 shadow-xs relative overflow-hidden">
          ...
          <p className="mt-1 text-[11px] text-secondary">
            {stats.primaryCurrency} {stats.paidAmount.toLocaleString()} confirmed
          </p>
        </div>
      </section>

      {/* Main Workspace: Filters & Invoices List */}
      <div className="rounded-3xl bg-surface border border-subtle p-6 shadow-sm space-y-6">
        {/* Filter & Search Bar */}
        <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4 pb-4 border-b border-subtle">
          {/* Status Tabs */}
          <div className="flex items-center gap-1.5 p-1 bg-elevated/80 rounded-xl border border-subtle overflow-x-auto">
            {[
          ...
          <div className="flex items-center gap-3 flex-wrap sm:flex-nowrap">
            <div className="relative flex-1 sm:w-60">
              ...
              <input
                ...
                className="w-full pl-9 pr-3 py-2 text-xs rounded-xl bg-elevated border border-subtle text-primary focus:outline-hidden focus:border-emerald-500/50 transition-colors"
              />
            </div>
            ...
            <select
              ...
              className="px-3 py-2 text-xs rounded-xl bg-elevated border border-subtle text-primary focus:outline-hidden focus:border-emerald-500/50 cursor-pointer"
            >
        ...
        <div className="space-y-4">
          {filteredInvoices.length === 0 ? (
            <div className="py-14 text-center space-y-3 bg-elevated/40 rounded-2xl border border-dashed border-subtle">
              ...
              <h3 className="font-bold text-sm text-primary">No Fee Challans Found</h3>
            ...
            filteredInvoices.map((inv) => {
              ...
              return (
                <div
                  key={inv.id}
                  className="rounded-2xl bg-elevated/50 hover:bg-elevated border border-subtle hover:border-emerald-500/40 p-5 transition-all duration-200 shadow-xs space-y-4"
                >
                  <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
                    {/* Left: University / Application details */}
                    <div className="flex items-start gap-4 min-w-0">
                      <div className="w-12 h-12 sm:w-14 sm:h-14 rounded-xl overflow-hidden border border-subtle shrink-0 shadow-xs relative">
                        <img
                          src={campusImage}
                          alt=""
                      ...
                      <div className="min-w-0 space-y-1">
                        <div className="flex items-center gap-2 flex-wrap">
                          <span className="font-mono font-bold text-xs sm:text-sm text-primary flex items-center gap-1.5">
                            <span>#{inv.invoiceNumber}</span>
                            <button
                              type="button"
                              onClick={() => copyToClipboard(inv.invoiceNumber, inv.id)}
                              className="text-muted hover:text-emerald-400 transition-colors p-0.5 cursor-pointer"
                              title="Copy Challan #"
                            >
                              {copiedId === inv.id ? (
                          ...
                          >
                            ...
                            />
                            {isPaid ? "Paid & Confirmed" : isPartiallyPaid ? "Proof Under Verification" : "Payment Pending"}
                          </span>
                        </div>
                    ...
                    <div className="flex flex-col sm:flex-row lg:flex-col sm:items-end justify-between gap-3 pt-3 lg:pt-0 border-t lg:border-t-0 border-subtle/50">
                      <div className="text-left sm:text-right">
                        <p className="text-xs font-semibold text-secondary">Amount Payable</p>
                        <p className="text-xl sm:text-2xl font-bold text-emerald-600 dark:text-emerald-400 font-heading">
                          {inv.currency} {inv.amount?.toLocaleString()}
                        </p>
                        <p className="text-[11px] text-muted flex items-center gap-1 sm:justify-end mt-0.5">
                      ...
                      <div className="flex items-center gap-2 flex-wrap sm:flex-nowrap">
                        <button
                          ...
                          className="px-3 py-1.5 rounded-xl bg-surface hover:bg-surface/80 border border-subtle text-xs font-semibold text-primary transition-all flex items-center gap-1.5 shadow-xs cursor-pointer"
                        >
                        ...
                        {isPaid && (
                          <div className="px-3 py-1.5 rounded-xl bg-emerald-500/10 border border-emerald-500/20 text-emerald-600 dark:text-emerald-400 font-bold text-xs flex items-center gap-1.5">
                  ...
                  {inv.notes && (
                    <div className="p-3 rounded-xl bg-surface/80 border border-subtle text-xs text-secondary leading-relaxed flex items-start gap-2">
      ...
      <section className="rounded-3xl bg-surface border border-subtle p-6 sm:p-7 shadow-sm space-y-4 relative overflow-hidden">
        <div className="flex items-center gap-2.5">
          ...
          <div>
            <h2 className="font-bold text-base text-primary font-heading">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 pt-2">
          <div className="p-4 rounded-2xl bg-elevated/50 border border-subtle space-y-1">
            <span className="text-[10px] uppercase font-bold text-muted tracking-wider">Beneficiary Name</span>
            <p className="font-bold text-sm text-primary font-mono">EduCRM Global Admissions Trust</p>
            <p className="text-[11px] text-secondary">Official Student Escrow & Settlement</p>
          </div>

          <div className="p-4 rounded-2xl bg-elevated/50 border border-subtle space-y-1">
            <span className="text-[10px] uppercase font-bold text-muted tracking-wider">Bank Name & Branch</span>
            <p className="font-bold text-sm text-primary">Standard Chartered Bank PLC</p>
            <p className="text-[11px] text-secondary">Global Education Admissions Division</p>
          </div>

          <div className="p-4 rounded-2xl bg-elevated/50 border border-subtle space-y-1">
            <span className="text-[10px] uppercase font-bold text-muted tracking-wider">Reference Requirement</span>
            <p className="font-bold text-sm text-emerald-600 dark:text-emerald-400 font-mono">
              Quote your Challan # & Student ID
            </p>
            <p className="text-[11px] text-secondary">Crucial for automated reconciliation</p>
          </div>
        </div>
      </section>
      ...
      {payingInvoice && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-xs animate-fade-in">
          <div className="w-full max-w-lg rounded-3xl bg-surface border border-subtle p-6 sm:p-7 space-y-5 shadow-2xl relative">
            <div className="flex items-center justify-between">
              ...
              <button
                ...
                className="p-1.5 rounded-xl text-secondary hover:text-primary hover:bg-elevated transition-colors cursor-pointer"
              >
                <X className="w-5 h-5" />
              </button>
            </div>
            ...
            <form onSubmit={handleProofSubmit} className="space-y-4 text-xs">
              <div>
                ...
                <div className="border-2 border-dashed border-subtle hover:border-emerald-500/50 rounded-2xl p-5 text-center transition-colors bg-elevated/30">
                  <input
                    type="file"
                    required
                    accept="application/pdf,image/jpeg,image/png,image/webp"
                    onChange={(e) => setProofFile(e.target.files?.[0] || null)}
                    className="w-full text-xs text-secondary file:mr-3 file:py-1.5 file:px-3 file:rounded-xl file:border-0 file:text-xs file:font-bold file:bg-emerald-500 file:text-zinc-950 hover:file:bg-emerald-400 file:cursor-pointer"
                  />
                  {proofFile && (
                    <p className="mt-2 text-xs font-semibold text-emerald-600 dark:text-emerald-400 flex items-center justify-center gap-1">
              ...
              <div>
                ...
                <input
                  ...
                  className="w-full px-3 py-2.5 rounded-xl bg-elevated border border-subtle text-primary focus:outline-hidden focus:border-emerald-500/50 text-xs"
                />
              </div>

              <div className="p-3.5 rounded-2xl bg-amber-500/10 border border-amber-500/20 text-amber-700 dark:text-amber-300 text-[11px] leading-relaxed">
              ...
              <div className="flex items-center justify-end gap-2.5 pt-2">
                <button
                  type="button"
                  onClick={() => setPayingInvoice(null)}
                  className="px-4 py-2.5 rounded-xl bg-elevated hover:bg-elevated/80 border border-subtle text-xs font-semibold text-secondary cursor-pointer"
                >
                  Cancel
                </button>
                <button
