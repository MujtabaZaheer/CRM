...
export const usePortalData = () => {
  ...
  useEffect(() => {
    ...
    const stops = [
      ...
      studentRole && !ownStudent ? noSubscription() : subscribe<Application>("applications", (x) => { setApplications(x.length > 0 ? x : DEMO_APPLICATIONS); done(); }, fail, studentRole ? "studentId" : undefined, studentRole ? ownStudent?.id : undefined),
      ...
      studentRole && !ownStudent ? noSubscription() : subscribe<PortalDocument>("student_documents", (x) => { setDocuments(x); done(); }, fail, studentRole ? "studentId" : undefined, studentRole ? ownStudent?.id : undefined),
      studentRole && !ownStudent ? noSubscription() : subscribe<VisaCase>("visa_cases", (x) => { setVisaCases(x.length > 0 ? x : (DEMO_VISA_CASES as unknown as VisaCase[])); done(); }, fail, studentRole ? "studentId" : undefined, studentRole ? ownStudent?.id : undefined),
      studentRole && !ownStudent ? noSubscription() : subscribe<SupportRequest>("support_requests", (x) => { setRequests(x); done(); }, fail, studentRole ? "studentId" : undefined, studentRole ? ownStudent?.id : undefined)
    ];
    return () => {
      clearTimeout(timeoutId);
      stops.forEach((stop) => stop());
    };
  }, [appUser?.email, appUser?.role, ownStudent?.id]);
